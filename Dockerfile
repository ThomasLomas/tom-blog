FROM ubuntu:trusty
MAINTAINER "Tom Lomas" <thomas.lomas@skybettingandgaming.com>

ADD run.sh /run.sh
ADD vhost.conf /etc/apache2/sites-available/php7.conf
ENV PATH /usr/local/php7/bin:$PATH

RUN chmod 775 /*.sh
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y wget curl apache2 libcurl4-openssl-dev libmcrypt-dev libxml2-dev libjpeg-dev libjpeg62 libfreetype6-dev libmysqlclient-dev libt1-dev libgmp-dev libpspell-dev libicu-dev librecode-dev
